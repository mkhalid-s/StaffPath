...
export const COMPANY_PACKS: Record<Exclude<CompanyPackId, 'none'>, CompanyPack> = {
  google: {
    ...
    chapterAngles: {
      ...
      'ai-evaluation-testing': "Google's AI systems are evaluated at massive scale — discuss offline eval sets, LLM-as-judge calibration, and how you gate model changes with regression suites before any production traffic shifts.",
      'prompt-engineering': "Google expects engineers to treat prompt changes like code changes — discuss versioning, A/B testing prompts, and how you measure whether a prompt change improved or degraded quality across user segments.",
      'api-gateway-patterns': "At Google scale, the API gateway handles millions of RPS — discuss hierarchical quota enforcement, global load balancing integrated with the gateway, and how you prevent the gateway from being a reliability bottleneck.",
      'event-driven-architecture': "Google Pub/Sub and Dataflow underpin many internal systems — discuss the trade-offs between choreography and orchestration at planetary scale and how you design for exactly-once semantics across distributed services.",
    },
  ...
  meta: {
    ...
    chapterAngles: {
      ...
      'ai-evaluation-testing': "Meta's AI features reach billions of users — discuss how you run rigorous A/B tests on model changes, measure quality across demographic segments, and use human evaluation to catch harms before they reach users at scale.",
      'prompt-engineering': "Meta ships AI features via iterative prompt tuning at speed — discuss how you build feedback loops between user behavior signals and prompt quality, and how you version prompts without slowing down the team's velocity.",
      'event-driven-architecture': "Meta's feed ranking and notification systems are event-driven at extreme scale — discuss fan-out on write vs fan-out on read, the celebrity problem in event systems, and how you handle schema evolution without breaking downstream consumers.",
      'api-gateway-patterns': "Meta serves multiple surfaces (Facebook, Instagram, WhatsApp) through different API tiers — discuss the BFF pattern for mobile vs web, how you enforce product-level rate limits without a centralized bottleneck, and how you instrument API quality by surface.",
    },
  ...
  netflix: {
    ...
    chapterAngles: {
      ...
      'ai-evaluation-testing': "Netflix's recommendation model changes affect 300M subscribers — discuss how you run holdout experiments, measure long-term engagement vs short-term click-through, and design evaluation pipelines that detect silent quality regressions.",
      'prompt-engineering': "Netflix uses LLMs for content discovery and metadata generation — discuss how you establish quality gates for AI-generated content, handle edge cases across a global content library, and version prompt changes with the same rigor as code releases.",
      'event-driven-architecture': "Netflix's microservices communicate primarily through events — discuss how you choose between choreography and orchestration for different business flows, how you trace causality through an event chain during an incident, and how you test event-driven systems without a full environment.",
      'api-gateway-patterns': "Netflix's API gateway (Zuul) handles global traffic with regional failover — discuss how you design a gateway that degrades gracefully under regional failures, supports per-user device capability routing, and maintains operational observability across a complex routing graph.",
    },
  ...
  startup: {
    ...
    chapterAngles: {
      ...
      'ai-evaluation-testing': "Startups shipping AI features need lightweight eval processes — discuss how you bootstrap an evaluation dataset with limited resources, use automated metrics to catch regressions without slowing down iteration, and decide when human evaluation is worth the cost.",
      'prompt-engineering': "At startup pace, prompts change daily — discuss how you avoid prompt regressions when moving fast, the minimum viable versioning process for a small team, and when structured outputs (JSON mode) are worth the added complexity over free-form generation.",
      'event-driven-architecture': "Startups often over-engineer event systems early — discuss when a simple message queue is the right choice versus a full event-driven architecture, the operational cost of managing an event broker with a small team, and how to sequence the adoption as the system grows.",
      'api-gateway-patterns': "A startup's API gateway should enable, not bottleneck — discuss choosing a managed gateway versus building your own, the minimum cross-cutting concerns worth centralizing in the gateway on day one, and when to migrate away from a shared gateway as teams scale.",
    },
  ...
  amazon: {
    ...
    chapterAngles: {
      ...
      'ai-evaluation-testing': "Amazon's Leadership Principles apply directly — discuss how you own the evaluation pipeline end-to-end (Ownership), Dive Deep into failure modes of LLM outputs, and define customer-centered metrics (Customer Obsession) rather than just technical accuracy scores.",
      'prompt-engineering': "Amazon treats prompt engineering as production software — discuss how you apply Dive Deep to understanding why a prompt fails, how Bias for Action shapes your iteration speed without sacrificing safety, and how you ensure prompt changes don't regress customer-facing quality.",
      'event-driven-architecture': "Amazon's internal services are famously loosely coupled through events — discuss how you design event schemas that are backward compatible across independent service release cycles, who owns the event contract, and how you handle event ordering guarantees at Amazon's scale.",
      'api-gateway-patterns': "Amazon API Gateway serves AWS's own services and customer workloads — discuss how you design a gateway that supports Frugality (minimal overhead per request), achieves the availability required by Customer Obsession, and gives teams the Ownership they need without central approval for each route.",
    },
  ...
  atlassian: {
    ...
    chapterAngles: {
      ...
      'ai-evaluation-testing': "Atlassian Intelligence must work across Jira, Confluence, and Bitbucket — discuss how you evaluate AI quality across different data domains (project data vs document data vs code), how you measure quality for developer workflows specifically, and how you build evaluation datasets that represent the breadth of Atlassian customer use cases.",
      'prompt-engineering': "Atlassian ships AI to enterprise customers who need predictability — discuss how you design system prompts that work across diverse enterprise data without hallucinating customer-specific facts, how you version prompts across a platform with multiple AI surfaces, and how you handle the 'play as a team' value when prompt ownership is shared across feature teams.",
      'event-driven-architecture': "Atlassian's platform integrates Jira, Confluence, Bitbucket, and third-party apps through events — discuss how you design a stable event schema for integration partners who have slow release cycles, how you version events without breaking existing integrations, and how you trace causality across product boundaries during a customer-impacting incident.",
      'api-gateway-patterns': "Atlassian's developer API serves thousands of third-party integrations — discuss how you design a gateway that enforces stable contracts for long-lived integrations, supports per-app rate limiting without penalizing well-behaved partners, and provides the developer experience (documentation, sandbox, error messages) that makes 'don't #@!% the customer' concrete.",
    },
