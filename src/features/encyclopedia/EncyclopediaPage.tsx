...
export function EncyclopediaPage() {
  ...
  return <div className="page encyclopedia-page">
    ...
    <div className="chapter-grid">{results.map((chapter) => {
      ...
      return <button className={`chapter-card ${packAngle ? 'has-pack-angle' : ''} ${isComplete ? 'completed' : ''}`} key={chapter.id} onClick={() => setSelected(chapter)}><div><span className="category-chip">{chapter.category}</span>{packAngle && <span className="pack-chip">{pack?.company}</span>}<span>{chapter.coreConcepts.length} concepts</span></div><h2>{chapter.title}</h2><p>{packAngle || chapter.summary}</p><ul>{chapter.coreConcepts.slice(0, 4).map((concept) => <li key={concept}>{concept}</li>)}</ul><strong>Open chapter →</strong></button>;
