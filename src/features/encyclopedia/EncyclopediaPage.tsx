...
export function EncyclopediaPage() {
  ...
  return <div className="page encyclopedia-page">
    ...
    <div className="chapter-grid">{results.map((chapter) => <button className="chapter-card" key={chapter.id} onClick={() => setSelected(chapter)}><div><span className="category-chip">{chapter.category}</span><span>{chapter.coreConcepts.length} concepts</span></div><h2>{chapter.title}</h2><p>{chapter.summary}</p><ul>{chapter.coreConcepts.slice(0, 4).map((concept) => <li key={concept}>{concept}</li>)}</ul><strong>Open chapter →</strong></button>)}{!results.length && <div className="empty-results"><strong>No chapters found</strong><p>Try a broader concept or another category.</p></div>}</div>
    {selected && <ChapterDetail chapter={selected} completed={state.completedChapters.includes(selected.id)} onToggleComplete={() => appStore.update((current) => ({ ...current, completedChapters: current.completedChapters.includes(selected.id) ? current.completedChapters.filter((id) => id !== selected.id) : [...current.completedChapters, selected.id] }))} onClose={() => setSelected(null)} />}
  </div>;
}
