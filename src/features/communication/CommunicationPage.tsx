...
export function CommunicationPage() {
  ...
  return <div className="page communication-page"><div className="page-heading"><div><p className="eyebrow">NONTECHNICAL MASTERY</p><h1>Communication Gym</h1><p>Practice presence, listening, conflict, feedback, storytelling, and influence as observable behaviors.</p></div><div className="progress-orb"><strong>{completed}/14</strong><span>lessons evidenced</span></div></div>
    <div className="communication-layout"><nav aria-label="Communication lessons">{communicationLessons.map((item, index) => <button className={`${item.id === selectedId ? 'active' : ''} ${state.communicationLessons[item.id]?.completedAt ? 'done' : ''}`} key={item.id} onClick={() => setSelectedId(item.id)}><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{item.title}</strong><small>{item.minutes} minute lesson</small></div><em>{state.communicationLessons[item.id]?.completedAt ? '✓' : '→'}</em></button>)}</nav>
